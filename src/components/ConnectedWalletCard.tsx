...
export default function ConnectedWalletCard({ wallet }: Props) {
  const { address, balanceSol, loading, error, connected, refreshBalance } = wallet;
  ...
  return (
    <Card className="glass-card-strong card-lift">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Your Wallet</CardTitle>
      ...
      <CardContent className="space-y-4">
        {!connected || !address ? (
          <div className="py-4 text-center">
            ...
            <p className="text-xs text-muted-foreground">Connect Phantom to view balance</p>
          </div>
        ) : (
          <>
            <div>
              ...
              <p className="mt-1 text-[10px] text-muted-foreground/50">Phantom wallet (devnet)</p>
            </div>
            ...
            <div className="flex items-center gap-2 rounded-lg bg-secondary/40 border border-border/40 px-3 py-2.5 group">
              ...
              <button
                onClick={handleCopy}
                className="ml-auto p-1.5 rounded-md hover:bg-muted/50 transition-colors focus-ring min-h-[28px] min-w-[28px] flex items-center justify-center"
                title="Copy address"
              >
                {copied
                  ? <Check className="h-3.5 w-3.5 text-success" />
                  : <Copy className="h-3.5 w-3.5 text-muted-foreground/50 group-hover:text-muted-foreground transition-colors" />
                }
              </button>
              <a
                ...
                className="p-1.5 rounded-md hover:bg-muted/50 transition-colors focus-ring min-h-[28px] min-w-[28px] flex items-center justify-center"
            ...
            <button
              ...
              disabled={loading}
              className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors disabled:opacity-40 focus-ring rounded-md py-1.5 min-h-[32px]"
            >
